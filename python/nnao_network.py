...
def build_nnao_network(print_shapes=False):
    ...
    conv_0d = tf.nn.conv2d(input=image_data, filter=weight_conv_0d, strides=CONV_STRIDES, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="convolution_0_down")
    relu_0d = tf.nn.relu(features=conv_0d + bias_conv_0d, name="relu_0_down")
    pooling_0d = tf.nn.avg_pool(value=relu_0d, ksize=POOL_CONFIG, strides=CONV_STRIDES, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_0_down")
    if print_shapes:
    ...
    pooling_1d = tf.nn.avg_pool(value=relu_1d, ksize=POOL_CONFIG, strides=CONV_STRIDES, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_1_down")
    if print_shapes:
    ...
    pooling_2d = tf.nn.avg_pool(value=relu_2d, ksize=POOL_CONFIG, strides=CONV_STRIDES, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_2_down")
    if print_shapes:
        print("2 Down: ", pooling_2d.shape)

    # Level 3 Network Bottom
    ...
    deconv_2u_shape = tf.stack([batch_size, int(relu_3d.shape[1]), int(relu_3d.shape[2]), CONV_2_SIZE])
    deconv_2u = tf.nn.conv2d_transpose(value=relu_3d, filter=weight_deconv_2u, output_shape=deconv_2u_shape, strides=DECONV_STRIDES, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="deconvolution_2_up")
    ...
    deconv_1u_shape = tf.stack([batch_size, int(relu_2u.shape[1]), int(relu_2u.shape[2]), CONV_1_SIZE])
    ...
    deconv_0u_shape = tf.stack([batch_size, int(relu_1u.shape[1]), int(relu_1u.shape[2]), CONV_0_SIZE])
