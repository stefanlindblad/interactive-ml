...
def build_nnao_network(print_shapes=False):
    image_data = tf.placeholder(dtype=tf.float32, shape=[None, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS], name="image_data")
    ground_truth = tf.placeholder(dtype=tf.float32, shape=[None, IMG_HEIGHT, IMG_WIDTH, 1], name="ground_truth")
    ...
    conv_0d = tf.nn.conv2d(input=image_data, filter=weight_conv_0d, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_0_down")
    relu_0d = tf.nn.relu(features=conv_0d + bias_conv_0d, name="relu_0_down")
    pooling_0d = tf.nn.avg_pool(value=relu_0d, ksize=POOL_CONFIG, strides=POOL_CONFIG, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_0_down")
    if print_shapes:
    ...
    conv_1d = tf.nn.conv2d(input=pooling_0d, filter=weight_conv_1d, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_1_down")
    relu_1d = tf.nn.relu(features=conv_1d + bias_conv_1d, name="relu_1_down")
    pooling_1d = tf.nn.avg_pool(value=relu_1d, ksize=POOL_CONFIG, strides=POOL_CONFIG, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_1_down")
    if print_shapes:
    ...
    conv_2d = tf.nn.conv2d(input=pooling_1d, filter=weight_conv_2d, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_2_down")
    relu_2d = tf.nn.relu(features=conv_2d + bias_conv_2d, name="relu_2_down")
    pooling_2d = tf.nn.avg_pool(value=relu_2d, ksize=POOL_CONFIG, strides=POOL_CONFIG, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_2_down")
    if print_shapes:
    ...
    conv_3d = tf.nn.conv2d(input=pooling_2d, filter=weight_conv_3d, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_3_down")
    relu_3d = tf.nn.relu(features=conv_3d + bias_conv_3d, name="relu_3_down")
    pooling_3d = tf.nn.avg_pool(value=relu_3d, ksize=POOL_CONFIG, strides=POOL_CONFIG, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="pooling_3_down")
    if print_shapes:
        print("3 Down: ", pooling_3d.shape)

    # Level 4 Network Bottom
    weight_conv_4d = tf.Variable(tf.truncated_normal([CONV_FILTER_SIZE, CONV_FILTER_SIZE, CONV_3_SIZE, CONV_4_SIZE], stddev=DEFAULT_DEV))
    bias_conv_4d = tf.Variable(tf.constant(DEFAULT_DEV, shape=[CONV_4_SIZE]))
    conv_4d = tf.nn.conv2d(input=pooling_3d, filter=weight_conv_4d, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_4_down")
    relu_4d = tf.nn.relu(features=conv_4d + bias_conv_4d, name="relu_4_down")
    if print_shapes:
        print("4 Bottom: ", relu_4d.shape)
    ...
    deconv_3u_shape = tf.stack([batch_size, int(relu_4d.shape[1])*2, int(relu_4d.shape[2])*2, CONV_3_SIZE])
    deconv_3u = tf.nn.conv2d_transpose(value=relu_4d, filter=weight_deconv_3u, output_shape=deconv_3u_shape, strides=DECONV_STRIDES, padding=PADDING_CONFIG, data_format=DATA_FORMAT_CONFIG, name="deconvolution_3_up")
    concat_3u = tf.concat(values=[deconv_3u, relu_3d], axis=3, name="concat_3_up")
    conv_3u = tf.nn.conv2d(input=concat_3u, filter=weight_conv_3u, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_3_up")
    relu_3u = tf.nn.relu(features=conv_3u + bias_conv_3u, name="relu_3_up")
    if print_shapes:
        print("3 ReLu: ", relu_3u.shape)
    ...
    conv_2u = tf.nn.conv2d(input=concat_2u, filter=weight_conv_2u, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_2_up")
    relu_2u = tf.nn.relu(features=conv_2u + bias_conv_2u, name="relu_2_up")
    if print_shapes:
        print("2 ReLu: ", relu_2u.shape)
    ...
    conv_1u = tf.nn.conv2d(input=concat_1u, filter=weight_conv_1u, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_1_up")
    relu_1u = tf.nn.relu(features=conv_1u + bias_conv_1u, name="relu_1_up")
    if print_shapes:
        print("1 ReLu: ", relu_1u.shape)
    ...
    conv_0u = tf.nn.conv2d(input=concat_0u, filter=weight_conv_0u, strides=CONV_STRIDES, padding=PADDING_CONFIG, use_cudnn_on_gpu=True, data_format=DATA_FORMAT_CONFIG, name="convolution_0_up")
    result = tf.nn.relu(features=conv_0u + bias_conv_0u, name="relu_0_up")
    if print_shapes:
        print("Result: ", result.shape)

    return result, image_data, ground_truth
